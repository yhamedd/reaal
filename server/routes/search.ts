...
searchRouter.get('/', requireAuth, (req, res) => {
  ...
  if (can(req.user, 'inventory.view')) {
    ...
    const wordClause = words.length > 1
      ? 'OR (' + words.map(() => "(p.name LIKE ? ESCAPE '\\' OR u.unit_number LIKE ? ESCAPE '\\' OR u.phase LIKE ? ESCAPE '\\' OR d.name LIKE ? ESCAPE '\\' OR u.property_type LIKE ? ESCAPE '\\')").join(' AND ') + ')'
      : '';
    const code = q.match(/^u-?0*(\d+)$/i);
    const params: any[] = [like, like, like, like, like, like, `%${unitNo ?? '\u0000'}%`];
    ...
    out.units = all<any>(
      ...
      `SELECT u.id, u.unit_number, u.phase, u.property_type, u.asking_price, u.status, u.bua, u.bedrooms, u.owner_id,
        ...
        WHERE u.archived_at IS NULL AND (
              p.name LIKE ? ESCAPE '\\' OR d.name LIKE ? ESCAPE '\\' OR u.unit_number LIKE ? ESCAPE '\\' OR u.phase LIKE ? ESCAPE '\\'
