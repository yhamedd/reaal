...
async function runValidation(db: import('../db.js').DB, imp: any, mapping: Record<string, string>, options: ImportOptions) {
  const headers: string[] = JSON.parse(imp.headers);
  const rows = JSON.parse(imp.rows);
  return await validateRows(db, imp.kind, headers, rows, mapping, options);
}
...
importsRouter.post('/:id/validate', requirePermission('imports.run'), async (req, res) => {
  ...
  const { summary, results } = await runValidation(db, imp, mapping, options);
  await run(db, "UPDATE imports SET mapping = ?, options = ?, summary = ?, status = 'validated' WHERE id = ?", [JSON.stringify(mapping), JSON.stringify(options), JSON.stringify(summary), imp.id]);
...
importsRouter.post('/:id/confirm', requirePermission('imports.run'), async (req, res) => {
  ...
  const { summary, results } = await runValidation(db, imp, mapping, cleanOptions(JSON.parse(imp.options || '{}')));
