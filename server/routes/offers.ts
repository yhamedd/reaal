...
offersRouter.post('/', requirePermission('offers.create'), async (req, res) => {
  ...
  const requirementId = Number(req.body?.requirement_id) || null;
  if (requirementId && !await get(db, 'SELECT 1 FROM requirements WHERE id = ?', [requirementId])) throw new HttpError(400, 'Unknown requirement');
  ...
  const id = await tx(db, async (db) => {
    ...
    for (const [i, u] of unitIds.entries()) await run(db, 'INSERT OR IGNORE INTO offer_units (offer_id, unit_id, position) VALUES (?, ?, ?)', [lastId, u, i]);
    const labels: string[] = [];
...
offersRouter.post('/pdf', requirePermission('offers.create', 'inventory.view'), async (req, res) => {
  ...
  const pdfUnits: PdfUnit[] = await Promise.all(units.map(async (u) => ({
    ...
    images: await Promise.all(
      (await all<any>(db, "SELECT * FROM files WHERE entity_type = 'unit' AND entity_id = ? AND mime IN ('image/png', 'image/jpeg', 'image/jpg') ORDER BY created_at", [u.id]))
        .filter((f) => s.pdf.show_images || f.category !== 'image')
        .map(async (f) => ({ data: await readStored(f.stored_name), mime: f.mime, category: f.category })),
    ),
  })));
  ...
  renderOfferPdf(res, {
    ...
    includeOwner,
    includePrice,
    logo: logo ? await readStored(logo.stored_name) : null,
  });
});
