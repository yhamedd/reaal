...
requirementsRouter.get('/', requirePermission('requirements.view'), async (req, res) => {
  ...
  if (q) {
    ...
    const parts = ["r.client_name ILIKE ? ESCAPE '\\'", "d.name ILIKE ? ESCAPE '\\'", "a.name ILIKE ? ESCAPE '\\'"];
    params.push(like, like, like);
    const phone = normalizePhone(q);
    if (phone.length >= 3 && /^[\d\s+()-]+$/.test(q)) {
      parts.push('r.phone_norm ILIKE ?');
      params.push(`%${phone}%`);
    }
...
requirementsRouter.post('/', requirePermission('requirements.manage'), async (req, res) => {
  ...
  const id = await tx(db, async (db) => {
    ...
    await logActivity(db, { userId: req.user!.id, action: 'created', entityType: 'requirement', entityId: lastId, label: data.client_name, message: `added a request for ${data.client_name}` });
    if (data.assigned_user_id !== req.user!.id) await notify(db, data.assigned_user_id, 'assigned', `${req.user!.name} assigned the request for ${data.client_name} to you`, `/requests/${lastId}`);
...
requirementsRouter.post('/:id/exclusions', requirePermission('requirements.manage'), async (req, res) => {
  ...
  for (const u of unitIds) {
    await run(db, 'INSERT INTO requirement_exclusions (requirement_id, unit_id, created_by) VALUES (?, ?, ?) ON CONFLICT DO NOTHING', [id, u, req.user!.id]);
  }
  if (unitIds.length) {
    const labels: string[] = [];
  ...
  await tx(db, async (db) => {
    ...
    if (keys.length) {
      ...
      if (data.assigned_user_id && data.assigned_user_id !== before.assigned_user_id && data.assigned_user_id !== req.user!.id) {
        await notify(db, data.assigned_user_id, 'assigned', `${req.user!.name} assigned the request for ${before.client_name} to you`, `/requests/${id}`);
      }
...
requirementsRouter.post('/:id/archive', requirePermission('requirements.manage'), async (req, res) => {
  ...
  await logActivity(db, { userId: req.user!.id, action: 'archived', entityType: 'requirement', entityId: id, label: r.client_name, message: `archived the request for ${r.client_name}` });
  res.json({ ok: true });
});
...
requirementsRouter.delete('/:id', requirePermission('records.purge'), async (req, res) => {
  ...
  await tx(db, async (db) => {
    ...
    await logActivity(db, { userId: req.user!.id, action: 'purged', entityType: 'requirement', entityId: id, label: r.client_name, message: `permanently deleted the request for ${r.client_name}` });
  });
  res.json({ ok: true });
});
