...
async function present(req: Express.Request, r: any, tags?: any[]) {
  const db = req.db;
  const projectIds: number[] = JSON.parse(r.preferred_project_ids || '[]');
  const projects = projectIds.length
    ? await all<any>(db, `SELECT id, name FROM projects WHERE id IN (${projectIds.map(() => '?').join(',')})`, projectIds)
    : [];
  const out = {
    ...r,
...
requirementsRouter.get('/', requirePermission('requirements.view'), async (req, res) => {
  const db = req.db;
  ...
  const tags = await getTags(db, 'requirement', rows.map((r) => r.id));
  // Match counts are cheap enough to compute per row for the list page.
  const out = [];
  for (const r of rows) {
    out.push({
      ...(await present(req, r, tags.get(r.id))),
      match_count: ['Active', 'Contacted'].includes(r.status) ? (await findMatches(db, r as RequirementRow)).length : null,
    });
  }
  res.json(out);
});

requirementsRouter.post('/', requirePermission('requirements.manage'), async (req, res) => {
  const db = req.db;
  ...
  const id = await tx(db, async (db) => {
    ...
    const { lastId } = await run(db, `INSERT INTO requirements (${cols.join(', ')}, created_by) VALUES (${cols.map(() => '?').join(', ')}, ?)`, [
      ...cols.map((c) => data[c]),
      req.user!.id,
    ]);
    if (Array.isArray(req.body?.tag_ids)) await setTags(db, 'requirement', lastId, idList(req.body.tag_ids));
    await logActivity(db, { userId: req.user!.id, action: 'created', entityType: 'requirement', entityId: lastId, label: data.client_name, message: `added a requirement for ${data.client_name}` });
    if (data.assigned_user_id !== req.user!.id) await notify(db, data.assigned_user_id, 'assigned', `${req.user!.name} assigned the requirement for ${data.client_name} to you`, `/requirements/${lastId}`);
    await markMatchesSeen(db, (await get<RequirementRow>(db, 'SELECT * FROM requirements WHERE id = ?', [lastId]))!);
    return lastId;
  });
  res.status(201).json({ id });
});

requirementsRouter.get('/:id', requirePermission('requirements.view'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  const r = await get<any>(db, `${SELECT} WHERE r.id = ?`, [id]);
  ...
  res.json({ ...(await present(req, r, (await getTags(db, 'requirement', [id])).get(id))), offers });
});

requirementsRouter.get('/:id/matches', requirePermission('requirements.view', 'inventory.view'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  const r = await get<RequirementRow>(db, 'SELECT * FROM requirements WHERE id = ?', [id]);
  if (!r) throw new HttpError(404, 'Requirement not found');
  const includeExcluded = req.query.include_excluded === '1';
  const offered = new Set(
    (await all<{ unit_id: number }>(db, 'SELECT DISTINCT ou.unit_id FROM offer_units ou JOIN offers o ON o.id = ou.offer_id WHERE o.requirement_id = ?', [id])).map((x) => x.unit_id),
  );
  const rows = await findMatches(db, r, { includeExcluded });
  const tags = await getTags(db, 'unit', rows.map((u) => u.id));
...
requirementsRouter.post('/:id/exclusions', requirePermission('requirements.manage'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  ...
  for (const u of unitIds) {
    await run(db, 'INSERT OR IGNORE INTO requirement_exclusions (requirement_id, unit_id, created_by) VALUES (?, ?, ?)', [id, u, req.user!.id]);
  }
  if (unitIds.length) {
    const labels: string[] = [];
    for (const u of unitIds) labels.push(await unitLabel(db, u));
    await logActivity(db, { userId: req.user!.id, action: 'matches_removed', entityType: 'requirement', entityId: id, label: r.client_name, message: `removed ${labels.join(', ')} from matches for ${r.client_name}` });
  }
  res.json({ ok: true });
});

requirementsRouter.delete('/:id/exclusions/:unitId', requirePermission('requirements.manage'), async (req, res) => {
  await run(req.db, 'DELETE FROM requirement_exclusions WHERE requirement_id = ? AND unit_id = ?', [intParam(req.params.id), intParam(req.params.unitId)]);
  res.json({ ok: true });
});

requirementsRouter.patch('/:id', requirePermission('requirements.manage'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  const before = await get<any>(db, 'SELECT * FROM requirements WHERE id = ?', [id]);
  ...
  await tx(db, async (db) => {
    ...
    if (keys.length) {
      await run(db, `UPDATE requirements SET ${keys.map((k) => `${k} = ?`).join(', ')}, updated_at = ? WHERE id = ?`, [...keys.map((k) => data[k]), nowIso(), id]);
      const b = { ...before };
      const a = { ...data };
      delete (a as any).preferred_project_ids;
      delete (a as any).property_types;
      await logChanges(db, req.user!, 'requirement', id, data.client_name ?? before.client_name, b, a, LOG);
      if (data.preferred_project_ids !== undefined && data.preferred_project_ids !== before.preferred_project_ids) {
        await logActivity(db, { userId: req.user!.id, action: 'updated', entityType: 'requirement', entityId: id, label: before.client_name, field: 'preferred_project_ids', message: `changed preferred projects for ${before.client_name}` });
      }
      if (data.property_types !== undefined && data.property_types !== before.property_types) {
        await logActivity(db, { userId: req.user!.id, action: 'updated', entityType: 'requirement', entityId: id, label: before.client_name, field: 'property_types', oldValue: before.property_types, newValue: data.property_types, message: `changed property types for ${before.client_name}` });
      }
      if (data.assigned_user_id && data.assigned_user_id !== before.assigned_user_id && data.assigned_user_id !== req.user!.id) {
        await notify(db, data.assigned_user_id, 'assigned', `${req.user!.name} assigned the requirement for ${before.client_name} to you`, `/requirements/${id}`);
      }
      // Criteria changed: current matches are the new baseline for "new match" alerts.
      await markMatchesSeen(db, (await get<RequirementRow>(db, 'SELECT * FROM requirements WHERE id = ?', [id]))!);
    }
    if (Array.isArray(req.body?.tag_ids)) await setTags(db, 'requirement', id, idList(req.body.tag_ids));
  });
  res.json({ ok: true });
});

requirementsRouter.post('/:id/archive', requirePermission('requirements.manage'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  const r = await get<any>(db, 'SELECT client_name FROM requirements WHERE id = ?', [id]);
  if (!r) throw new HttpError(404, 'Requirement not found');
  await run(db, 'UPDATE requirements SET archived_at = ?, updated_at = ? WHERE id = ?', [nowIso(), nowIso(), id]);
  await logActivity(db, { userId: req.user!.id, action: 'archived', entityType: 'requirement', entityId: id, label: r.client_name, message: `archived the requirement for ${r.client_name}` });
  res.json({ ok: true });
});

requirementsRouter.delete('/:id', requirePermission('records.purge'), async (req, res) => {
  const db = req.db;
  const id = intParam(req.params.id);
  const r = await get<any>(db, 'SELECT * FROM requirements WHERE id = ?', [id]);
  if (!r) throw new HttpError(404, 'Requirement not found');
  ...
  await tx(db, async (db) => {
    await deleteEntityFiles(db, 'requirement', id);
    await run(db, "DELETE FROM notes WHERE entity_type = 'requirement' AND entity_id = ?", [id]);
    await run(db, "DELETE FROM taggings WHERE entity_type = 'requirement' AND entity_id = ?", [id]);
    await run(db, 'DELETE FROM requirement_exclusions WHERE requirement_id = ?', [id]);
    await run(db, 'DELETE FROM requirement_match_seen WHERE requirement_id = ?', [id]);
    await run(db, 'UPDATE offers SET requirement_id = NULL WHERE requirement_id = ?', [id]);
    await run(db, 'DELETE FROM requirements WHERE id = ?', [id]);
    await logActivity(db, { userId: req.user!.id, action: 'purged', entityType: 'requirement', entityId: id, label: r.client_name, message: `permanently deleted the requirement for ${r.client_name}` });
  });
  res.json({ ok: true });
});
