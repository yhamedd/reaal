...
export async function queryUnits(db: DB, user: AuthUser, rawFilters: unknown, rawSort: unknown, limit: number, offset = 0) {
  ...
  const rows = await all<any>(db, `${UNIT_SELECT} ${where} ${orderBy(parseSort(rawSort), UNIT_SORTS, DEFAULT_UNIT_ORDER)} LIMIT ? OFFSET ?`, [...p, limit, offset]);
...
unitsRouter.get('/', requirePermission('inventory.view'), async (req, res) => {
  const limit = Math.min(Number(req.query.limit) || 2000, process.env.VERCEL ? 5000 : 50000);
...
unitsRouter.post('/bulk', requirePermission('inventory.edit', 'inventory.bulk_edit'), async (req, res) => {
  ...
  await tx(db, async (db) => {
    for (const id of ids) {
      ...
      for (const t of addTags) await run(db, "INSERT INTO taggings (tag_id, entity_type, entity_id) VALUES (?, 'unit', ?) ON CONFLICT DO NOTHING", [t, id]);
    }
    await logActivity(db, {
      userId: req.user!.id,
      action: 'bulk_updated',
