...
export function seedDemo(db: DB, { units = 240 } = {}) {
  if (get<{ n: number }>(db, 'SELECT COUNT(*) AS n FROM units')!.n > 0) return false;
  let seed = 42;
  const rand = () => ((seed = (seed * 1664525 + 1013904223) % 4294967296) / 4294967296);
  const pick = <T,>(arr: T[]) => arr[Math.floor(rand() * arr.length)];

  tx(db, () => {
    const roles = new Map(all<any>(db, 'SELECT id, key FROM roles').map((r) => [r.key, r.id]));
    const team = [
    ...
    for (const [name, email, role, phone] of team) {
      if (!get(db, 'SELECT 1 FROM users WHERE email = ?', [email])) {
        run(db, 'INSERT INTO users (name, email, phone, password_hash, role_id) VALUES (?, ?, ?, ?, ?)', [name, email, phone, hashPassword('Password123'), roles.get(role)]);
      }
    }
    const userIds = all<{ id: number }>(db, 'SELECT id FROM users').map((u) => u.id);
    const projects = all<any>(db, 'SELECT id, name, developer_id FROM projects');
    ...
    run(db, "INSERT INTO activity (user_id, action, message) VALUES (?, 'seeded', 'loaded demo data')", [userIds[0]]);
  });
  return true;
}
